...
const EditBrandUI = ({item, setOpen}) => {
  ...
  useEffect(() => {
    if (item) {
      ...
      if (item.logoUrl) {
        setImagePreview(`https://smartteamaz-001-site1.qtempurl.com${item.logoUrl}`);
      }
    }
  }, [item]);
  ...
  const removeImage = () => {
    setImageFile(null);
    setImagePreview(item?.logoUrl ? `https://smartteamaz-001-site1.qtempurl.com${item.logoUrl}` : null);
  };
